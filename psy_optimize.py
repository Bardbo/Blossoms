...
def optimize(algorithm,
    ...
    if prophet is not None:
        if type(prophet) == np.ndarray:
            ...
            if type(algorithm.population) == ea.Population:
            ...
            elif type(algorithm.population) == ea.PsyPopulation:
                prophetPopChroms = []
                for encoding, encoidx, field in zip(algorithm.population.Encodings, algorithm.population.EncoIdxs, algorithm.population.Fields):
                    if encoding == 'RI' or encoding == 'P':
                        prophetPopChroms.append(prophet[:, encoidx])
                    elif encoding == 'BG':
                        prophetPopChroms.append(ea.ri2bs(prophet[:, encoidx], field))
                    else:
                        raise RuntimeError('error in optimize: The encoding should be ''RI'', ''P'' or ''BG''. (种群编码必须为''RI'', ''P'' 或 ''BG''。)')
                prophetPop = ea.PsyPopulation(algorithm.population.Encodings, algorithm.population.Fields, 1, prophetPopChroms)
        elif type(prophet) == ea.Population:
            prophetPop = prophet.copy()
        elif type(prophet) == ea.PsyPopulation:
             prophetPop = prophet.copy()
        else:
            raise RuntimeError('error in optimize: The type of prophet must be Numpy ndarray or Population or PsyPopulation. (prophet的类型必须为Numpy ndarray数组或种群类。)')
    # 参数设置
